...
##### get mean magnitude image ##########
mags=' '.join(magnitude)
mrg_cmd=f'{fsldir}/bin/fslmerge -t mean_magnitude.nii.gz {mags}'
sp.run(mrg_cmd.split())
µ_cmd=f'{fsldir}/bin/fslmaths mean_magnitude.nii.gz -Tmean magnitude.nii.gz'
sp.run(µ_cmd.split())
...
#######compute linear regression as performed by Windischberger et.al  PMID: 15390143 DOI: 10.1002/jmri.20158

### genrate the  numerator
print("#####PERFORMING LINEAR REGRESSION #########")
UW=gl.glob('*unwrapped*gz')
#getting resolution 
res_cmd=f'{fsldir}/bin/fslinfo {UW[0]}'
res_info=sp.run(res_cmd.split(),stdout=sp.PIPE)
res_info=res_info.stdout.decode('utf-8')
res_info=(re.findall("[-+]?\d+\.\d+",res_info))
res_info=res_info[0:3]
for i in res_info:
	i=float(i)
res_info=np.asarray(res_info)
...
# ######## calculate epi aquisition matrix 
# fov=fov.astype(np.float)
# res_info=res_info.astype(np.float)

# epi_mat=np.divide(res_info,fov)

# print(fov)
# print(res_info)

# ##### epimat as numerator in eqn 14 in paper
# epi_mat=1/epi_mat
# print(epi_mat)

# ######## calculate bandwidth
# const3T=0.22
# Bw=(const3T*epi_mat)/wfs

# num=(epi_mat/Bw)
# print(num)
# num=num[0]
# fmap=np.multiply(deltaF,num)

# fmap=np.multiply(fmap,(2*np.pi))
fmap=np.multiply(fmap,1000)
...
fg_cmg="fugue --loadfmap=fieldmap -s 2 --savefmap=fieldmap_s2"
sp.run(fg_cmg.split())
